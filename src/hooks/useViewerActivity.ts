  ...
  const loadViewerActivity = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        // ВЛАСНІ перегляди відсікаються НА СЕРВЕРІ, і це не дублювання
        // клієнтського фільтра нижче, а виправлення порядку.
        //
        // `recordPropertyView` пише ВСІХ, включно з власником, тож без цього
        // предиката вікно в 500 рядків забивалось його ж власними
        // відкриттями — а відсіювались вони вже ПІСЛЯ ліміту. Власник із
        // сорока обʼєктами набирає стелю за кілька днів, і блок «Хто
        // переглядав» показував порожньо, хоч активність рієлторів у таблиці
        // була. Тобто дефект тим гірший, чим активніше користуються базою.
      ...
      // Клієнтський відсів ВЛАСНИХ переглядів лишається другою сіткою: на
      // сервері вже стоїть `.neq`, але цей рядок дешевий і тримає інваріант
      // навіть якщо предикат колись загубиться в рефакторингу.
