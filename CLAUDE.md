...
- **`screenshots.spec.ts` — ВІЗУАЛЬНИЙ БЕЙСЛАЙН, а не тур.** 27 кадрів усіх
  ...
  **Оновлювати ЛИШЕ під `CI=1`** (`CI=1 npx playwright test screenshots
  --update-snapshots`): CI ганяє прод-білд під `serve`, dev-сервер рендерить не
  піксель-в-піксель те саме, і бейслайн, знятий у dev, валить CI. Наступали:
  dev-регенерація перезаписала `share-sheet` кадром, що відрізнявся від
  прод-рендера на ~8px по вертикалі, хоч CSS того шита ніхто не чіпав.
