...
AA дозволено на екран (db-list 0, db-objects 5, property-detail 0, profile 4), тож
...
борг: лише `.view-seg-b` (38px по ширині).

**ПАСТКА, через яку «виправлене» роками не працювало.** `::after` обрізається
будь-яким предком із `overflow ≠ visible`, і тоді зона дотику лишається розміром
бокса, хоч у CSS виглядає розширеною. Так було з `.seg-b` (власний
`overflow:hidden` під `text-overflow:ellipsis`) і `.notif-tab` (контейнер
`.notif-tabs` має `overflow-x:auto`, що за спекою робить і `overflow-y`
не-visible): обидва «мали» 44px, а фактично 34 і 38. Їм підняли РЕАЛЬНУ висоту
падінгом і забрали з `::after`-правила; лишились там тільки `.view-seg-b`,
`.fr-seg-b`, `.hdr-back`, де нічого не клiпає.
