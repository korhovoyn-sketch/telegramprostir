...
- **`?.` НЕ досить: нижня смуга кидає СИНХРОННО.** `MainButton`/`SecondaryButton` відкидають невалідний параметр винятком (`WebAppBottomButtonParamInvalid`), а з ефекту він летить у ErrorBoundary — замість форми користувач бачить «Щось пішло не так». Тому кожен виклик іде через `tgSafe()` у `useMainButton`. Найгостріший випадок: **порожній/пробільний `text`**. Плейсхолдер `text: ' '` для схованої кнопки завалив створення об'єкта у проді — якщо підпису немає, параметри не надсилаємо взагалі, лише `hide()`. Стаб харнеса валідує так само строго, тож регресія падає в тестах.
