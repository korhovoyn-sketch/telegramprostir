...
- **`screenshots.spec.ts` — ВІЗУАЛЬНИЙ БЕЙСЛАЙН, а не тур.** 27 кадрів усіх
  ...
  **БЕЙСЛАЙНОМ ВОЛОДІЄ CI, і локально його НЕ ПЕРЕЗНІМАТИ — ніколи, ні з `CI=1`,
  ні без.** Порівняння кадрів залежить від ЗБІРКИ Chromium, а вони різні по боках:
  ...
  браузера, тож розбіжність невидима, доки не побіжить CI — саме так `main`
  простояв **червоним три мерджі підряд** (#92–#94), і кожен наступний PR
  успадковував чуже падіння. Тому пісочниця спеку `screenshots` ПРОПУСКАЄ
