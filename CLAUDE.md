...
- **E2e**: Playwright, один проєкт `iphone-se` (375×667), 168 тестів. Харнес `tests/e2e/helpers/harness.ts` — стаб `window.Telegram.WebApp` (initData, CloudStorage, BackButton) + повний мок Supabase REST/Auth через `page.route`. Тести НЕ ходять у мережу.
...
`tests/e2e/_contrast.spec.ts` (не входить у прогін; `PERF=1 npx playwright test
_contrast`) рахує контраст по РЕАЛЬНОМУ рендеру: знімок робиться двічі —
...
**Залишковий борг (заміряно, 21 блок нижче AA):** решта не лікується альфою
тексту, бо це не текстові токени, а фон під ним:
- `.obj-tot-*` і `.obj-tot-v` — білий bold на 3.84:1 над **світлим тонованим**
  блоком підсумку: треба темніший блок, брайтити нікуди;
- `.bdg-busy` / `.bdg-info` — тонований бейдж, foreground на тлі своєї ж заливки;
- `.fr-seg-b` — неактивний сегмент 2.39:1 (мова/валюта в Профілі);
- 13 правил із власним білим-з-альфою лишаються в allowlist гарда — тост,
  галерея, сплеш, hero, превʼю експорту, неактивна іконка таббару.

**Тап-таргети:** гард `ui-audit` стоїть на 32px, хоч проєкт цитує Apple HIG
44×44. Заміряний розрив — 22 контроли по 34-36px висоти. Виправлено там, де
безпечно: `.seg-b`, `.view-seg-b`, `.fr-seg-b`, `.notif-tab`, `.hdr-back`
отримали невидимий `::after` (`top:50%;translate:0 -50%;height:44px`) — ВИГЛЯД
рядків не змінився, зона дотику виросла до 44px. Перевірено `elementFromPoint`
по центру й краях розширеної зони на трьох екранах — 0 випадків, де вона краде
тап у сусідній контрол.
...
- `.view-seg-b` (перемикач «Картки/Компактно», 38×34): квадратні кнопки-іконки
  впритул одна до одної в спільній обгородці — розширення вкрало б тап у сусідню.
...
- Тестова база: 189 unit + 168 e2e; спільні тест-хелпери в harness.ts.
