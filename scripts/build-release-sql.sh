...
# ── ЧУЖА ТРАНЗАКЦІЯ ВСЕРЕДИНІ БАНДЛА ──────────────────────────────────────
# Postgres НЕ має вкладених транзакцій: `BEGIN` усередині відкритої дає лише
# WARNING і не робить нічого, а от `COMMIT` — закриває ЗОВНІШНЮ. Тобто одна
# міграція, що керує транзакцією сама, розриває транзакцію всього файлу, і
# все, що йде після неї, виконується в autocommit.
#
# Це не теорія: 065 несла власні BEGIN/COMMIT, і замір показав, що після збою
# в кінці файлу обʼєкти 064 і 065 ЛИШАЮТЬСЯ в базі. Тобто головна обіцянка
# шапки — «при будь-якій помилці НІЧОГО не застосується» — була хибною саме
# для 066, БЕЗПЕКОВОЇ міграції. Оператор при цьому бачить помилку й, за вже
# записаним уроком, вважає, що не застосувалось нічого.
#
# Вирізаємо тут, а не вимагаємо від автора міграції нічого не забути: цей
# генератор і так не є чистою конкатенацією (він додає передпольотну
# перевірку й блок DROP), а правило, яке треба памʼятати, повертається.
# Сама міграція свою обгортку зберігає — вона потрібна їй при ОКРЕМОМУ накаті.
#
# Сканер стежить за доларовим цитуванням ($$ і $tag$), тому `BEGIN`/`END`
# усередині тіла функції чи DO-блоку не чіпає. Без цього ми б різали plpgsql.
strip_own_tx() {
  awk -v mode="${2:-STRIP}" '
    function consume(rest,   idx) {
      while (1) {
        if (intag == "") {
          if (match(rest, /\$[A-Za-z_][A-Za-z0-9_]*\$|\$\$/)) {
            intag = substr(rest, RSTART, RLENGTH)
            rest  = substr(rest, RSTART + RLENGTH)
          } else return
        } else {
          idx = index(rest, intag)
          if (idx == 0) return
          rest  = substr(rest, idx + length(intag))
          intag = ""
        }
      }
    }
    BEGIN { intag = ""; nb = 0; nc = 0 }
    {
      if (intag == "" && toupper($0) ~ /^[[:space:]]*BEGIN;[[:space:]]*$/)  { nb++; if (mode == "STRIP") next }
      if (intag == "" && toupper($0) ~ /^[[:space:]]*COMMIT;[[:space:]]*$/) { nc++; if (mode == "STRIP") next }
      consume($0)
      if (mode == "STRIP") print
    }
    END { if (mode != "STRIP") print nb, nc }
  ' "$1"
}
...

{
  ...
  echo "--     застосується, тобто напівстану не буде. Це не декларація: CI"
  echo "--     підставляє збій у зоні ОСТАННЬОЇ міграції і вимагає, щоб у базі"
  echo "--     не лишилось обʼєктів попередніх (verify-migrations.sh);"
...
# ── ДОКАЗ, А НЕ НАДІЯ ──────────────────────────────────────────────────────
# Після зшивання на верхньому рівні мусить лишитись РІВНО одна пара
# BEGIN/COMMIT — та, яку додає цей генератор. Рахуємо тим самим сканером, що
# й вирізав чужі: якщо він десь помилився, різниця видно тут, а не в проді.
begins=$(strip_own_tx "$OUT" COUNT)
if [ "$begins" != "1 1" ]; then
  echo "✗ у зібраному файлі не одна пара BEGIN/COMMIT, а «$begins» —" >&2
  echo "  тобто транзакція десь розривається і накат перестає бути атомарним." >&2
  exit 1
fi
