...
// рядків — edge-функція `send-reminders`, і типів у неї два: `rent_reminder`
...
test('вкладка «Договори»: свій тип лишається, чужі — ні', async ({ page }) => {
  // Гард на два різні дефекти одразу.
  //
  // Перший, історичний: гілки для `lease` у фільтрі не було взагалі, тож він
  // падав у фінальний `return true` — вкладка показувала ВСІ сповіщення
  // поспіль, тобто не фільтрувала нічого.
  //
  // Другий — рівно дзеркальний, і саме його легко внести, лагодячи перший:
  // гілка `return false` фільтрує «правильно» рівно доти, доки виробника
  // рядків не існує. Відколи 065 його дала, голий `false` ховає САМЕ ті рядки,
  // заради яких вкладка й потрібна. Тому перевіряються ОБИДВІ половини.
  await setup(page, [
    notif(1, { type: 'rent_reminder', title: 'Платіж за Офіс 101' }),
    notif(2, { type: 'view', title: 'Перегляд обʼєкта 2' }),
    notif(3, { type: 'lease_reminder', title: 'Договір: Офіс 103' }),
  ])
  await openApp(page)
  await page.locator('.tabbar [aria-label="Сповіщення"]').click()
  await expect(page.getByText('Платіж за Офіс 101')).toBeVisible({ timeout: 15_000 })
  ...
  await expect(page.getByText('Договір: Офіс 103'), 'нагадування про договір — це і є ця вкладка').toBeVisible()
...
test('ГЛОБАЛЬНИЙ лічильник, випущений РАНІШЕ, не воскрешає бейдж', async ({ page }) => {
  // Гонка, яку відтворюємо: запитів на `notifications` ДВА і від різних
  // власників — глобальний лічильник бейджа в `page.tsx` (на
  // `requestIdleCallback`) і сам екран (`load().then(markAllAsRead)`). Обидва
  // пишуть у стор УВЕСЬ список, тож виграє той, хто відповів ОСТАННІМ, а не
  // той, чиї дані свіжіші.
  //
  // ПОРЯДОК ТУТ КЕРОВАНИЙ ТЕСТОМ, а не таймерами. Попередня редакція будувала
  // його на снах (2.5с на випуск + 6с утримання + 4.5с на приземлення) і через
  // це падала в ПОВНОМУ паралельному прогоні на CI, лишаючись зеленою локально
  // 3/3: `requestIdleCallback` за специфікацією відкладається під
  // навантаженням, а всі запаси там були вгадані, не гарантовані.
  //
  // Тепер перший запит ВИСИТЬ, доки тест сам його не відпустить — саме тоді,
  // коли вже видно `markAllAsRead` на дроті. Ніяких перегонів із планувальником
  ...
  await page.route('**/rest/v1/notifications**', async (r) => {
    ...
    if (m === 'DELETE') { wire.deletes.push(r.request().url()); return json(r, []) }
    const n = ++gets
    // Перший (глобальний) — тримаємо до явного дозволу. Решта — миттєво.
  ...
  await openApp(page)
  // Антивакуум: глобальний запит МУСИТЬ бути випущений до кліку, інакше першим
  // виявиться запит екрана і гард перевірятиме не ту послідовність.
  await expect.poll(() => gets, {
    message: 'глобальний лічильник не випустив запит — гард був би вакуумним',
    timeout: 15_000,
  }).toBeGreaterThan(0)
