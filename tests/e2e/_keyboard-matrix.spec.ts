 ...
 * перекриття клавіатура зʼїдає низ вікна, у режимах стискання вікно вже без неї.
 */
...
test('інші шити з полями × клавіатура на двох найпоширеніших екранах', async ({ page }) => {
  ...
  // без `.modal` узагалі. «Запросити гостя» прибрано так само (фаза 3):
  // InviteSheet замінений повноекранним CreateInviteScreen. Цикл нижче явно
  // чекає на `.modal`, тож інструмент, чиє призначення саме клавіатура В
  // МОДАЛКАХ, тут не узагальнюється під геометрію екрана. Еквівалентне
  // покриття — генерично в field-obstruction.spec.ts/keyboard-viewport.spec.ts.
...
test('шит не ПЕРЕЛІТАЄ своє місце, поки відкривається клавіатура', async ({ page }) => {
  ...
  for (const dev of [DEVICES[0], DEVICES[2], DEVICES[3], DEVICES[6]]) {
    for (const mode of ['overlay', 'resize-reported', 'resize-silent'] as Mode[]) {
      ...
      // ОБОВʼЯЗКОВА синхронізація ПЕРЕД стартом заміру. Харнес хардкодить
