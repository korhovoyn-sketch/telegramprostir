...
const CONTRAST_DEBT: ReadonlySet<string> = new Set([
  ...
  // Капшен переваги на екрані входу. Нижче порога лише ТРЕТІЙ із трьох —
  // він стоїть найнижче, а градієнт світлішає донизу.
  'feature-s',
...
const FROZEN: Record<string, number> = {
  // ПЕРШИЙ екран продукту. Один блок заморожено, другий ВИПРАВЛЕНО, і межа
  // між ними не смакова: `.feature-s` — капшен переваги («Три кроки до першої
  // бази»), а рядок згоди з Умовами й Політикою — текст, на який користувач
  // ДАЄ ЗГОДУ самим натисканням кнопки, тобто не підпис (той самий критерій,
  // за яким підняли «Пропустити →»). Йому дали `--t3`; капшен лишається.
  //
  // Показово, що з трьох `.feature-s` нижче порога лише ТРЕТІЙ: градієнт
  // світлішає донизу, тож це рівно той ефект «вертикальне положення — бюджет
  // контрасту», що вже описаний у CLAUDE.md.
  'welcome': 1,
  ...
  // наявний борг видимим і незростаючим ще на одному екрані.
  'edit-db': 2,
  'empty-state': 0,
  'db-list': 0,
  // ТРИ, а не два, і це число З CI, а не з локального прогону.
  //
  // Пісочниця міряє тут 2 (`dash-sub` ×2), раннер — 3, і третій належить уже
  // ПРИЙНЯТОМУ класу: перевірка «НОВИЙ текст нижче AA» на CI проходить, падає
  // лише лічильник. Причина та сама, що в бейслайнів: контраст рахується по
  // РЕАЛЬНОМУ рендеру (знімок + семпл пікселів), а Chromium 141 у пісочниці й
  // 148 на раннері растеризують текст по-різному — значення на волосині від
  // 4.5 перевертається.
  //
  // Тобто `FROZEN` має ту саму сліпоту, що й локальне перезняття кадрів:
  // ЛОКАЛЬНИЙ ПРОГІН НЕ Є АВТОРИТЕТОМ. Я вже подав «348 passed, 0 failed» як
  // готовність — це було правдою про машину і неправдою про CI.
  'db-objects': 3,
