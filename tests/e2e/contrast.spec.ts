...
const FROZEN: Record<string, number> = {
  ...
  'db-objects': 6,
  'property-detail': 0,
  // `.over>span` + 5 × `.fr-seg-b` (мова/валюта). Було 4 (`.over>span` + 2×
  // `.fr-seg-b`), стало 6 після виправлення висоти `#app-root` (раніше
  // рахувався від `var(--tg-vh)` — Telegram-мок, а не справжнє вікно — тож
  // екран був на ~99px коротшим). Довший екран зсуває контент нижче в
  // ГРАДІЄНТІ, що світлішає донизу (див. розділ «Читабельність» вище): ті самі
  // ВЖЕ задокументовані класи (не нові!) тепер перетинають поріг на 2 елементи
  // більше — «USD»/«Укр» не дотягують по 0.04–0.19, а не тому що контраст
  // раптом упав самостійно.
  profile: 6,
}
 ...
 *
 * `.obj-act-btn` — рядок дій картки («Редагувати/Дублювати/Платежі/Файли»).
 * Природний бокс (padding 7px + 12px шрифт) — це ~35px, і `::after` тут СВІДОМО
 * не розширюють (див. globals.css, розділ «ЗОНА ДОТИКУ»): над кнопками — тіло
 * картки, яке відкриває об'єкт, і розширена зона вкрала б у нього тапи. Це
 * рішення вже було ухвалено; просто цей гард ніколи РЕАЛЬНО його не перевіряв —
 * `smallTargets()` відсікає все, що лежить під `.tabbar` (fold), а до фіксу
 * `#app-root`-висоти (див. `fix(kb)`) TabBar сам стояв на ~99px вище своєї
 * справжньої позиції, тож рядок дій завжди потрапляв «за фолд» і пропускав
 * перевірку. Тепер `#app-root` correct, і гард нарешті бачить те, що й так
 * було на екрані.
 *
 * Окремо, гірший випадок: коли перша картка в базі — з довгою назвою, орендарем
 * і платіжним підсумком (як у фікстурі цього тесту), її рядок дій географічно
 * потрапляє в 46px-зону плаваючої кнопки «Додати об'єкт» (`.fbtn.raised`) — між
 * кінцем картки (y≈593) і верхом TabBar (y≈611) лишається лише 18px, менше за
 * висоту самої FAB. Жодним зміщенням `bottom`-офсету це не розв'язати (сплив
 * при спробі: підняти близько до розкладу — кнопка лягає ПОВНІСТЮ на рядок дій
 * замість часткового накриття). Реальний фікс — зменшити висоту картки в цьому
 * densest-стані (тенант+лізинг+підсумок одночасно) — окрема робота, не гідна
 * поспіху під час цього мерджу.
 */
const TAP_DEBT: ReadonlySet<string> = new Set(['view-seg-b', 'obj-act-btn'])
