...
/**
 * ВЛАСНІ перегляди відсікає СЕРВЕР, а не лише клієнт.
 *
 * Тест вище доводить, що клієнтський фільтр працює, — і саме тому він був
 * зелений на зламаному коді: мок віддає фікстуру цілком, ігноруючи
 * `.limit(500)`, тож вичерпання вікна відтворити рендером НЕМОЖЛИВО В
 * ПРИНЦИПІ. А в проді `recordPropertyView` пише всіх, включно з власником:
 * його ж відкриття забивали 500 рядків, після чого чужа активність
 * відсіювалась разом із ними, і блок був порожній тим певніше, чим активніше
 * користувач.
 *
 * Отже міряти треба не список, а ЗАПИТ — те саме рішення, що в
 * `collection-analytics` (там лічильник на екрані лишався правильним і зі
 * зламаним фільтром, бо мок віддає той самий масив на будь-який запит).
 */
test('запит виключає власні перегляди на СЕРВЕРІ, а не лише на клієнті', async ({ page }) => {
  const urls: string[] = []
  await setupApp(page, { user: OWNER })
  await skipCoachmarks(page)
  ...
  // АНТИВАКУУМ: без цього «жоден запит не порушив правило» означало б
  // «запитів не було».
  expect(urls.length, 'блок мусив сходити по перегляди').toBeGreaterThan(0)

  const q = decodeURIComponent(urls.join(' '))
  expect(q, 'предикат neq мусить бути в ЗАПИТІ, інакше власні перегляди '
    + 'займають вікно ліміту й витісняють чужі').toContain(`viewer_id=neq.${OWNER.id}`)
})
