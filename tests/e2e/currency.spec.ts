...
test('валюта переживає перезапуск (кеш профілю)', async ({ page }) => {
  ...
  // Активний сегмент — стан ОПТИМІСТИЧНИЙ, тобто зʼявляється ДО того, як PATCH
  // доїде; «переживає перезапуск» має сенс лише коли сервер валюту вже прийняв.
  // Тому релоуд робимо після підтвердженого PATCH, а не після класу `on`.
  //
  // ЧЕСНО про межу цього рядка: він НЕ доведений як лікування флейку, який один
  // раз упав на CI (картка лишалась «$2 460» усі 15с, пройшло на ретраї).
  // Гіпотезу «релоуд обриває PATCH у льоті» відтворити НЕ вдалось: зі штучною
  // затримкою відповіді 600мс і БЕЗ цього очікування тест усе одно проходив.
  // Тобто механізм лишається невідомим, а це — прибирання однієї правдоподібної
  // гонки і уточнення передумови тесту, не закритий баг.
  await expect.poll(() => captured.patch?.currency, { timeout: 10_000 }).toBe('EUR')
