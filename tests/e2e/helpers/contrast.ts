...
export const smallTargets = (page: Page, min: number) => page.evaluate<SmallTarget[], number>((m) => {
  ...
  document.querySelectorAll('button,[role="button"],a,select,input[type="checkbox"],.sheet-row,.notif-tab,.seg-b,.view-seg-b,.fr-seg-b,.tab,.obj-more,.hdr-back').forEach((el) => {
    ...
    // ШИРИНУ МІРЯЄМО ТАК САМО, ЯК ВИСОТУ, і до цього раунду — НІ.
    // Зонд брав `r.width` сирим боксом, тобто розширення зони через `::after`
    // зараховувалось лише по вертикалі. Асиметрія давала помилки В ОБИДВА
    // боки: контрол із розширеною зоною репортувався як малий (степер
    // кількості — «32×42» при реальних 44×44), а справжній дефіцит ширини
    // ніде не компенсувався. Саме на цьому стояв запис `.view-seg-b`
    // («38px по ширині») — він описував ваду ЗАМІРУ, а не контрола.
    const hitsX = (x: number) => {
      if (x < 1 || x > window.innerWidth - 1) return false
    ...
    while (left < 30 && hitsX(cx - left - 1)) left++
    while (right < 30 && hitsX(cx + right + 1)) right++
    const effH = up + down + 1
    const effW = left + right + 1
    if (effW < m || effH < m) {
      out.push({
 ...
 *
 * **`'button'` ЗВІДСИ ПРИБРАНО, і це був найгірший запис списку.** Він
 * звільняв не клас, а ТЕГ — тобто кожна кнопка без класу в усьому застосунку
 * була невидима для порога 44px, і причини поруч не стояло жодної (три його
 * сусіди мають розгорнуте пояснення). Знайдено це рівно тоді, коли в обхід
 * додали екран `welcome`: його кнопка «Діагностика підключення» мала 199×28.5
 * і не потрапляла в жоден звіт. Бланкетне звільнення за іменем тега — це той
 * самий антипатерн, що вже коштував раунду у файловому allowlist-і
 * `mutation-discipline`, лише на рівень грубіший.
 *
 * `legal-a` — посилання на Умови й Політику ВСЕРЕДИНІ речення згоди (15–16px
 * заввишки). Підняти їх до 44 означає роздути рядковий бокс абзацу і посунути
 * первинну дію екрана; це рішення про верстку першого екрана, а не про сам
 * контрол. Клас заведено САМЕ щоб борг був іменований і обмежений — доти вони
 * рахувались як `a`, тобто звільнення накрило б усі посилання застосунку.
 *
...
export const TAP_DEBT: ReadonlySet<string> = new Set([
  'view-seg-b', 'sort-chip', 'owner-act', 'legal-a',
  // Чипи діапазону календаря («1 міс … 6 міс», фільтр). Реальну висоту
  // піднято 28 → 38, але 44 тут недосяжні: на 360px рядок ПЕРЕНОСИТЬСЯ, і
  // розширення `::after` по вертикалі крало б тапи сусіднього рядка —
  // заміряно, ефективна висота падала до 34. Горизонталь у них уже ≥56.
  'range-chip',
