...
for (const lang of LANGS) {
  test.describe(`воркфлоу — ${lang}`, () => {
    ...
    test('платежі: сума МІСЯЧНА, а не сира ставка', async ({ page }) => {
      ...
      // АНТИВАКУУМ, І ВІН ЖЕ ОЧІКУВАННЯ. Одноразовий `innerText` тут ГОНИВСЯ
      // із завантаженням: ЗАГОЛОВОК малюється раніше за розклад, тож під
      // навантаженням замір ловив порожній календар («0 ПРОСТРОЧЕНО / —
      // ОТРИМАНО»). Заміряно: 4 падіння з 8 під `--repeat-each=4`. Локатор із
      // авто-ретраєм робить ту саму перевірку детермінованою і лишається
      // антивакуумом — без суми на екрані перевірка нижче проходила б на
      // порожньому.
      await expect(page.getByText(/1[\s\u00a0\u202f,]?0?80/).first())
