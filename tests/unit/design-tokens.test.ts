...
//
// Гард навмисно розрізняє два випадки, бо вони НЕ однакові:
//  • автоматичний фокус при відкритті (таймер/ефект у компоненті шита) — зась;
//  • `autoFocus` на полі, яке зʼявляється ВІД ТАПУ користувача всередині вже
//    відкритого шита (розкрити «+ Нова папка», тапнути «Перейменувати») — це
//    пряма відповідь на дію, і вона лишається.
describe('клавіатура в модалках', () => {
  it('жоден шит не фокусує поле сам при відкритті', () => {
    ...
    for (const file of SHEETS) {
      const txt = stripComments(readFileSync(file, 'utf8'))
      // Фокус із таймера/ефекту = автоматичний. Саме його прибрали.
