...
// Невідомий CSS-токен НЕ помітний під час ревʼю: `background:var(--bg2)` виглядає
// нормально, але браузер робить властивість невалідною — фон стає прозорим, а
// рамка зникає. Так панель дій над обраними обʼєктами і екран помилки місяцями
...
// обовʼязково вживається з фолбеком — інакше він нічим не кращий за помилку.
...
const OWN_TEXT_ALPHA = new Set([
  ...
  // Сплеш і hero обʼєкта — власна мова над орбом/фото.
...
describe('шкала кольору тексту', () => {
  ...
  /**
   * ЯСКРАВО-ЗЕЛЕНИЙ «ГОТОВО» МАЄ ІМʼЯ, І ВОНО ОДНЕ.
   *
   * `#4ade80` жив 15 входженнями у 7 файлах (галочки завершення, рамка
   * завантаженого фото, аватар гостя, підпис «Збережено») при тому, що поруч
   * уже існують `--ok` і `--ok-fg` з ІНШИМИ значеннями. Тобто це була де-факто
   * конвенція без імені — рівно те, що правило «рецепт, написаний УТРЕТЄ, —
   * це токен» описує, лише перевищене упʼятеро.
   *
   * Чому джерельний гард, а не рантаймовий: `design-system-runtime` міряє
   * РЕЗУЛЬТАТ, а тут результат однаковий за побудовою — токен має те саме
   * значення, що й літерал. Дрейф видно лише в ДЖЕРЕЛІ.
   *
   * ЧОМУ ЦЕ НЕ ДУБЛЬ сусіднього «колір, який уже має токен, не переписують
   * літералом у JSX» — той озброївся САМ, щойно зʼявився `--ok-bright`
   * (він звіряє літерали зі значеннями токенів). Але він свідомо пропускає
   * `OWN_PALETTE` і все, що не `.tsx`, — а саме там і жила половина входжень:
   * ЧОТИРИ в `src/app/v/page.tsx` (файл у `OWN_PALETTE` через нативний хром,
   * якому потрібен голий hex) і одне в самому `globals.css`. Тобто цей гард
   * бачить рівно те, чого не бачить сусідній, а не повторює його.
   */
  it('яскраво-зелений «готово» береться з токена, а не з літерала', () => {
    const files = walk('src')
    const offenders: string[] = []
    for (const f of files) {
      const txt = readFileSync(f, 'utf8')
      for (const line of txt.split('\n')) {
        if (!line.includes('#4ade80')) continue
        // Єдине легальне місце — оголошення самого токена в `:root`.
        if (line.includes('--ok-bright:')) continue
        offenders.push(`${f}: ${line.trim().slice(0, 80)}`)
      }
    }
    expect(offenders, 'беріть var(--ok-bright) — інакше тон розповзається копіями, як це вже сталось із 15 входженнями').toEqual([])
  })
})

describe('типографіка', () => {
  /**
   * ОДИН АПОСТРОФ НА ВЕСЬ ЗАСТОСУНОК.
   *
   * Рендерилось ТРИ різні символи: `&apos;` (37), U+02BC `ʼ` (36), U+0027 `'`
   * (32) і один U+2019 `’`. Вони помітно різні на екрані — «Обʼєктів» на «Мої
   * бази» проти «об'єкти» на «Команда бази», — і жоден гард цього не бачив, бо
   * кожен окремо валідний.
   *
   * Обрано U+02BC: Unicode рекомендує саме його як БУКВЕНИЙ апостроф
   * української (U+0027 — машинописна лапка, U+2019 — типографська лапка), він
   * уже становив більшість, і в JSX не потребує `&apos;`-екранування, тобто
   * прибирає ще й причину, через яку варіанти розходились.
   *
   * Гард стежить ЛИШЕ за апострофом МІЖ кириличними літерами: лапки в англійських
   * рядках, `don't` у коментарі чи ' як роздільник рядка коду — не його справа.
   */
  it('апостроф в українських словах — лише U+02BC', () => {
    const CYR = 'а-щьюяіїєґА-ЩЬЮЯІЇЄҐ'
    const wrong = new RegExp(`[${CYR}](?:&apos;|['’])[${CYR}]`)
    const bad: string[] = []
    for (const file of files) {
      const txt = readFileSync(file, 'utf8')
      txt.split('\n').forEach((line, i) => {
        const m = line.match(wrong)
