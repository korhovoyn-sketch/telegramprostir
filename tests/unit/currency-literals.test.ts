...
/**
 * Захардкоджений «$» у підписі — баг, який НЕ видно на ревʼю і не видно в
...
function stripNonCurrency(src: string): string {
  return src
    ...
    .replace(/^\s*\/\/.*$/gm, '')
    .replace(/\$\{/g, '')
    // regex-літерал: /…/flags — там «$» це якір кінця рядка, не валюта.
    // ЛУКБІХАЙНД ОБОВʼЯЗКОВИЙ: без нього косою рискою всередині РЯДКА
    // (`'image/'`) починався фальшивий regex, він зʼїдав шматок до наступної
    // риски, і справжній regex лишався надкушеним — з «$» назовні. Саме так
    // гард репортував неіснуючий дефект у `PropertyDetailScreen`.
    // Рядки при цьому НЕ вирізаються: «$» усередині них і є те, що ловимо.
    .replace(/(?<![A-Za-z0-9_)\]'"`])\/(?![/*])(?:\\.|\[(?:\\.|[^\]])*\]|[^/\n\\])+\/[gimsuy]*/g, '')
}
