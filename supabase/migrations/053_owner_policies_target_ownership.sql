...
-- пара до 052.
--
-- ── ГІПОТЕЗА, СПРОСТОВАНА ТУТ ЖЕ ───────────────────────────────────────────
-- `collection_properties` має ту саму форму (перевіряє лише свою підбірку),
-- тож напрошується «рієлтор покладе чужий обʼєкт у свою підбірку й опублікує».
-- Вставка справді проходить — але `get_public_collection_preview` віддає
-- НУЛЬ рядків для такого обʼєкта, тобто публічної поверхні в цього немає, і
-- власний екран рієлтора теж відфільтрує його по RLS. Політику не чіпаємо.
-- ============================================================================
